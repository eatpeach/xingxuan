...
| 项目 | 内容 |
|---|---|
| **状态** | ✅ 文档部分全部完成（含 README）· 🔴 **但扫出一条需 CTO 立即处理：`admin123` 是 seed 活代码，见结论 5.2** |
| **负责人** | 开发人员A |
| **指派人** | CTO |
| **创建时间** | 2026-08-09 |
| **时限** | 2026-08-12 前 |
...
#### ~~🔴 但有一件事删不掉，需要 CTO 决策：**`admin123` 仍在 git 历史里**~~ → ✅ **已结案**

> **结果（老板 08-09 答复）**：新密码**不是** `admin` 系列变体，历史里那条是失效且与现口令无构造关联的旧口令。
> **决策：不重写历史，保留原样。** 本条到此关闭，详见文末裁决。以下为当时的分析留档。
...
### 五、🔴 追加（08-09 第二轮）：README 已修，但扫出一件推翻既有结论的事

#### 5.1 `README.md` —— 默认账号有**两处**，且比 `CLAUDE.md` 更旧

CTO 只点了 `README.md:33`，实际是**两处**：第 33 行（`php -S` 注释里）和**第 50 行**
（「默认账号：`admin / admin123`。首次登录后请到系统设置改密码（密码修改 action 待加）」）。
**只删 33 行会漏掉 50 行**，两处都已删。

README 整体过期程度超过 `CLAUDE.md` 前半部分，已重写。除账号外核出的错：

| 处 | 原文 | 实际 |
|---|---|---|
| 48 行 | 「Vite 已把 `/api` 反代到 `127.0.0.1:8000`，前后端同时跑就能登录」 | **反代到生产站**（`vite.config.ts:11-12`）。照原文理解会以为 `npm run dev` 连的是本地，实际**直接读写生产库**——这条比账号更容易出事故，已加 ⚠️ 显著标注 |
| 50 行 | 「密码修改 action 待加」 | 已有 `changePassword`（`handler.php:90`） |
| 89 行 | 公开 action 3 个 | 11 个，且漏了 `$vendorActions` 一整层 |
| 59 行 | 「导出 PDF（待接）」 | 已实现（html2canvas + jsPDF） |
| 105–110 行 TODO | 「PDF 渲染」「客户公开询价 H5」「供应商 Excel 批量导入」列为待办 | **三项都已完成**，TODO 段整段删除 |
| 75–83 行 | 系统设置 5 项 | **29 项**，已按 6 组重列 |
| 目录树 / API 表 | 停在早期 | 已补货架 / 供应商门户 / 商品库 / 收款主体 / 订单履约链 |

#### 5.2 🔴 **`admin123` 不是失效口令——它是 seed 里的活代码**
...
if ($cnt === 0) {
    $hash = password_hash('admin123', PASSWORD_BCRYPT);   // ← 硬编码
    $st->execute(['admin', $hash, '管理员', 'admin']);      // ← admin 角色
}
```

`seed()` 由 `initialize()` 调用，**每次 API 请求都可能走到**。

**这推翻了本单先前的结论。** 之前认定「历史里的 `admin123` 是失效口令、不重写历史」，
前提是它只存在于文档和 git 历史。实际上**它是当前 `main` 分支上的可执行代码**，
公开仓库任何人都能读到，而且创建的是 **admin 角色**。

**风险取决于一件可查的事：`users` 表里现在还有没有 `username = 'admin'` 这一行。**

| 情况 | 后果 |
|---|---|
| 老板是**改了 admin 这个账号的密码**（行还在，用户名仍是 `admin`） | 闸门成立，seed 不会重建。**风险确实低**，先前结论成立 |
| 老板是**删了 admin、另建了别的用户名** | 🔴 **下一个 API 请求就会用 `admin123` 重建一个管理员账号**，而口令公开可读 |

**请 CTO 确认后一种情况有没有发生**——后台「系统设置 → 账户管理」看一眼用户列表即可，几秒钟。
若 `admin` 这个用户名已经不存在，**这是一个当前可利用的入口，不是历史遗留**。

**我没有动这行代码**：改 seed 属于代码改动（本单红线：只改文档），且涉及部署行为，是老板的决策。
处置方案若要我做，请另开单。

> 顺带：同一个 `seed()` 里 `bank_account_no` / `bank_account_name` 的默认值也是真实值。
> CTO 已就账号那条上报老板并交代「别自己动」，**账户名同在一处**，一并决策为好。我没碰。
...
# ✅ CTO 裁决（2026-08-09，回应 A 关于 `admin123` 仍在 git 历史里）

## 一、**不重写历史。** 你的判断和理由都对

- 密码**已经改过**，泄露的是一个**失效口令**，真实风险是「暴露账号命名习惯 + 曾用弱口令风格」，
  不是「现在能被登进去」
- 重写公开仓库历史要 force push，会改写所有已推提交的 hash：A、B **共用同一个工作区**，
  还有在途改动，代价明显大于这点残留信息的收益
- 你没有自行处理是对的——**破坏性操作必须老板拍板，开发不能自己动手**

（补充一条背景：**今天已经重写过一次历史了**，为清理混进台账的跨项目信息。
同一个公开仓库短时间内反复 force push，对任何已有 clone 都是持续的对齐成本。
这也是不该为一个已失效口令再来一次的理由。）

## 二、你提的那条确认，我采纳并已上报老板

> 新密码是不是 `admin` 系列的变体（`admin1234` / `admin@123` 之类）？
> 如果是，历史里泄露的就不只是旧口令，而是**现口令的构造规律**。

这条抓得准——**这才是真风险所在**。已转达老板，并声明：
**不要在台账、任务单或任何文件里回复新密码本身**，只回答「是 / 不是同一构造」。

## 三、🔴 但这两件都是治标，根问题我已上报老板：**这个仓库该不该公开**

删一行、重写历史，都是在处理「密码泄露」这一个症状。而 `github.com/eatpeach/xingxuan`
公开着的还有：

- `deploy.sh`（服务器路径、部署流程）
- 完整 DB schema（34 张表、全部字段与外键）
- 全部 API 结构与公开 action 白名单
- 业务逻辑全文

**这些不是靠删一行能解决的，而且没有一样是删得掉的——它们本来就是代码。**

真正的决策是：**星选的后端代码要不要公开。** 已上报老板，等答复。
在他决定之前，**你在 09 号单里加的那句「本仓库是公开的，不要往里写账号/密钥/内部排期/其它项目信息」
非常必要**，那是当前唯一的防线。

## 四、09 号单本身

六处过期的修正已看到（`04185db`）。**等你把交付清单走完，我做终审。**

---

## ✅ 密码确认已回（2026-08-09，老板本人答复）

**新密码不是 `admin` 系列变体。**

→ A 提的那条风险**关闭**：历史里泄露的是一个**已失效、且与现口令无构造关联**的旧口令。
剩余风险仅剩「暴露过账号命名习惯」，**不值得为它重写公开仓库历史**。

~~**`admin123` 保留在 git 历史里，不做处理。本条决策到此结案。**~~

> ## 🔴 2026-08-10 CTO 撤回上面这条的**理由**（结论保留，理由作废）
>
> A 查出 `admin123` **不只在文档和 git 历史里，它是活代码**：
> `database.php:890-895` 的 `seed()` 在 `users` 表没有 `admin` 行时会用它重建管理员账号，
> 而 `seed()` 由 `handler.php:26` 在**鉴权之前**调用——
> **任何一个请求（含无需登录的公开接口）都会触发**。
>
> 老板 08-10 已确认 `users` 表里 `admin` 行还在 → **即时风险排除**，
> 但防线只有一道数据条件，失守时无任何告警，且密码公开可查、新部署天生带后门。
>
> **「不重写历史」的结论仍然成立，但理由必须换掉**：
> 不是因为口令已失效，而是**重写历史根本没用**——`admin123` 就在当前源码里，清历史清不掉它。
> **唯一有效的处置是改代码** → [10 号单](20260810-10-seed-hardcoded-admin.md)。
>
> 📌 留个教训：我当时把「文档里写着（用户已改）」当成了「这个口令在系统里已经失效」，
> **没有去代码里确认它是不是还在被使用**。判一个凭据死没死，要看代码，不能看文档怎么说。

（回答只记录「不是」这个结论，**新密码本身没有出现在任何文件或消息里**——这是 A 要求的处理方式，照做。）

**仍未决的是根问题**：`github.com/eatpeach/xingxuan` 该不该公开
（`deploy.sh` 服务器路径、34 张表完整 schema、全部 API 结构与业务逻辑都在里面，**删不掉，它们就是代码**）。
已第三次上报老板，等答复。在他决定前，09 号单加的那句「本仓库是公开的，不要往里写账号/密钥/内部排期/其它项目信息」是唯一防线。
