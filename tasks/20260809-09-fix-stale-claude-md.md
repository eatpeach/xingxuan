...
#### 🔴 但有一件事删不掉，需要 CTO 决策：**`admin123` 仍在 git 历史里**

```
git log --oneline -S "admin123"        → 7 个提交
git show 8a9fff2:CLAUDE.md | grep ...  → 明文可读
```

**`github.com/eatpeach/xingxuan` 是公开仓库，任何人 clone 下来都能从历史里读到这个密码。**
删当前文件**只挡住了直接浏览的人，挡不住看历史的人**。

**我没有自行处理，因为唯一的彻底办法是重写 git 历史**（`filter-repo` / BFG + **force push**），
那是破坏性操作：会改写所有已推提交的 hash，A、B 共用的工作区和任何已有 clone 都要重新对齐，
还可能冲掉别人在途的改动。**这种事必须 CTO / 老板拍板，开发不能自己动手。**

**我的判断是：不用重写历史，但要确认一件事。**

- 老板**已经改过密码**了（这是单子背景里就写明的），所以泄露的是一个**已失效的口令**，
  真实风险是「暴露了账号命名习惯 + 曾用弱口令风格」，不是「现在能被登进去」
- 重写公开仓库历史的代价（两个开发的工作区全部要重来、在途改动有丢失风险）
  **明显大于**这点残留信息的收益

**→ 唯一需要确认的是：新密码是不是 `admin` 系列的变体**（比如 `admin1234`、`admin@123`）。
如果是，那历史里泄露的就不只是旧口令，而是**现口令的构造规律**——那就必须改密码（改密码即可，
仍然不需要重写历史）。**这条请 CTO 转达老板确认，不要在台账或任何文件里回复新密码本身。**
