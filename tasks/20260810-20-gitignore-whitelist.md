...
## 附：四个数据脚本的本地真 PHP 实跑（B，2026-08-10，CTO 批准）

装了 PHP 之后补做。**全程本地 seed 临时库，跑完删库删备份，未接触生产。**

| 脚本 | 结果 |
|---|---|
| `backfill_invoice_snapshots.php` | ✅ 见 [07 号单](20260809-07-backfill-invoice-snapshots.md)（dry-run/apply/幂等/11 项数据核对/备份还原） |
| **`purge_all_products.php`** | ✅ **删全表 + 回滚路径首次被验证**，见下 |
| `audit_quote_cascade_risk.php` | ✅ 真 PHP 下退出码 0，输出正常，跑完库未被改动（只读性成立） |
| `audit_settings_change_history.php` | ✅ 同上 |

### 🔴 `purge_all_products.php`：删全表脚本的回滚路径，这是第一次被验证

它**已经在生产上跑过**（01 号单，清空商品库），但它的备份能不能真还原，此前从没验过。

本地造 25 条商品 + 10 条改价记录：

- dry-run 输出正确（按状态/供应商/品类的分组统计都对得上）
- `--apply` 删除 25 条商品、10 条改价记录，自增 id 归零
- **照脚本自己打印的回滚办法**（`cp` 备份 → 删同名 `-wal`/`-shm`）还原：
  **商品 25 条、改价 10 条、演示数据 8 条、品类 3 个，精确还原；库结构完整 35 张表**

→ **「有备份所以可回滚」这句话，对这个脚本也从承诺变成了事实。**

### 一个我自己的失误，记一笔

清理测试文件时 shell 引号写错，在 `backend/data/` 下造出一棵带空格的垃圾目录树。
**已发现并整棵删除**，里面全是我自己造的测试文件，无真实数据，`git status` 全程干净。

值得记的是：**它当时没被 `git status` 报出来，正是因为本单的新规则把它忽略了**——
换成改规则之前，那棵树里的 `x.sqlite3`、`凭证.jpg`、`图纸.pdf` 会以未跟踪文件的身份
出现在 `git status` 里，一次 `git add -A` 就进公开仓库了。**算是新规则的一次意外实战验证。**
