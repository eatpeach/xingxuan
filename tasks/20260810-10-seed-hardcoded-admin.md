...
**`bank_name`（`'BCA'`）** 我最初按「单子点名两项、别顺手多改」留着，把去留交 CTO。
**CTO 裁决：清掉**（`57e620c` 之后的追加提交）。理由比「是不是识别性数据」更本质——
看 `InvoicePrint.tsx:302`，**银行名和账号是各自独立渲染的**：清账号留 `'BCA'` 会印出
「银行：BCA / 账号：空」，**半填的银行块比全空更糟**（看着像真实收款信息却不可用，客户无从转账）。
判断标准是「清了之后剩下的还成不成立」——三项是一组，要么都留要么都清。已改空。
