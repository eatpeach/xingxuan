...
### 后端（本机已有 PHP —— 2026-08-10 起）

**本机装了 PHP 8.5.9**（`/opt/homebrew/bin/php`，可能不在 PATH 里，用全路径）。
扩展齐全：`pdo_sqlite` / `sqlite3` / `mbstring` / `fileinfo` / `gd` / `curl`。

> ⚠ 这一节 2026-08-10 之前写的是「本机没有 PHP」。**旧的每一张任务单里都还印着那句话**，
> 看到了以本节为准。

**现在能做的：**

```bash
/opt/homebrew/bin/php -l 文件.php        # 真语法检查，别再手数括号
/opt/homebrew/bin/php scripts/data-fixes/xxx.php   # 数据脚本可本地跑
```

- **改完 PHP 必须 `php -l`**。此前只能靠手工数括号配平，那是权宜之计，现在没理由不做
- **`PDO 占位符数 = execute 参数数` 仍要自己查** —— `php -l` 只验语法，验不出这个，
  它依然是本项目最常翻车的点
- 数据脚本可以在**本地 seed 出来的临时库**上先跑一遍，不必拿生产当第一现场

🔴 **两个版本差异要注意：**

1. **本地 8.5，生产 8.2。** 别用 8.3+ 才有的语法——本地过了生产会挂
...
- 登录用 seed 出来的账号：用户名 **`admin`**，密码**每次新建空库时随机生成**（10 号单改的，
  不再是硬编码的 `admin123`）。密码写在 **`backend/data/initial-admin-password.txt`**（已 gitignore，不进仓库），
  `cat` 一下就能拿到。库已存在时不会重建、也不会重写这个文件——
  **换句话说：第一次 `php -S` 起来、随便打一个接口触发 seed 之后，去读那个文件拿密码。**
  首次登录后可在「系统设置」改密码
